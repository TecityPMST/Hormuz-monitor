...
const GAS_ANNOTATIONS = {
  ...
  afterDatasetsDraw(chart){
    ...
    // --- peak markers ---
    function drawPeak(dates, vals, color, labelAbove){
      let peakIdx = 0;
      ...
      ctx.fillStyle = color;
      const tooCloseToTop = py - top < 20;
      ctx.textAlign = px > x.right - 90 ? 'right' : 'left';
      const tx = ctx.textAlign === 'right' ? px - 7 : px + 7;
      ctx.fillText(text, tx, tooCloseToTop ? py + 16 : py - 11);
    }
    drawPeak(aaaDates, aaaVals, '#1f3a5f');
    drawPeak(doeDates, doeVals, '#c77f2e');
...
function renderGasChart(){
  ...
  gasChartInstance = new Chart(ctx, {
    ...
    options:{
      ...
      layout:{ padding:{ right: 78, top: 16 } },
      plugins:{legend:{labels:{font:{size:9}}}},
      scales:{x:{type:'category', ticks:{font:{size:8}}, grid:{display:false}}, y:{min:2.5,max:5.5, ticks:{font:{size:9}}, grid:{color:'#e9edf2'}}}
    }
  });
}
